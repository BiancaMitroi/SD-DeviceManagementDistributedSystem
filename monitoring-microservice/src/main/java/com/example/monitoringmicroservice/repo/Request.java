...
public class Request {
    ...
    private JSONObject convertChatMessage(String message, String userMail) {
        JSONObject jsonObject = new JSONObject();
    ...
    private void sendMessageToWebSocket(SensorData sensorData, String userMail) {
        ...
        assert device != null;
        String apiUrl = "http://chat-microservice:8083/api/v1/chat";
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        if(sensorData.getValue() >= device.getMaximumConsumption()) {
            JSONObject jsonObject = this.convertChatMessage("The value " + sensorData.getValue() +
    ...
    public ResponseEntity<String> getUser(Integer id) {
        String apiUrl = "http://users-microservice:8080/api/v1/users/getUser";
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
